...
Known open items: balance tuning, and the optional role/round HUD element in `cl_dll/` described as step 4 of `dlls/README.md`.

Spawning is handled by a `GetPlayerSpawnSpot` override rather than `EntSelectSpawnPoint`, because the stock picker telefrags whoever is on a spawn point once it runs out of free ones - and `StartPreRound()` spawns the whole server in one tick. The override gathers every `info_player_*` plus `maps/<map>_spawns.txt` (`x y z [yaw]` per line), takes the first spot a player hull fits in, then probes a ring around taken spots for floor before falling back to upstream. The Hunter is placed after the Killer and prefers spots at least `CH_ROLE_SPAWN_SEPARATION` away.
